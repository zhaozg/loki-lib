...
template <class A, class T> void destroy(A &a, T *p, typename A::size_type n) {
...
void destroy_range(ForwardIterator b, ForwardIterator e) {
  yasli_nstd::type_selector<yasli_nstd::is_class<typename std::iterator_traits<
                                ForwardIterator>::value_type>::value != 0,
                            _impl::destroyer,
...
template <class It1, class It2> It2 uninitialized_move(It1 b, It1 e, It2 d) {
  return mojo::uninitialized_move(b, e, d);
}

template <class A> struct generic_allocator_traits {
  static typename A::pointer reallocate(A &a, typename A::pointer b,
  ...
  static bool reallocate_inplace(A &a, typename A::pointer b,
                                 typename A::size_type newSize) {
    return false;
  }
...
struct allocator_traits<yasli::allocator<T>>
  ...
  static bool reallocate_inplace(A &a, typename A::pointer b,
                                 typename A::size_type newSize) {
    allocator_traits<yasli_nstd::mallocator<T>>::reallocate_inplace(a, b,
                                                                    newSize);
  }

  static typename yasli::allocator<T>::pointer
  reallocate(yasli::allocator<T> &a, typename yasli::allocator<T>::pointer b,
...
struct allocator_traits<yasli_nstd::mallocator<T>>
  ...
  static bool
  reallocate_inplace(yasli_nstd::mallocator<T> &a,
  ...
  reallocate(yasli_nstd::mallocator<T> &a,
             typename yasli_nstd::mallocator<T>::pointer b,
    ...
    if (reallocate_inplace(a, b, newSize))
      return b;
    return generic_allocator_traits<yasli_nstd::mallocator<T>>::reallocate(
        a, b, e, newSize);
  }
};
} // namespace yasli_nstd
...
namespace _impl {
// safe
template <class InputItr, class FwdItr> struct uninitialized_safe_copier {
...
template <class T> struct uninitialized_memcopier {
...
FwdItr uninitialized_copy(InputItr first, InputItr last, FwdItr result) {
  std::cout << "neither\n";
  return _impl::uninitialized_safe_copier<InputItr, FwdItr>::execute(
      first, last, result);
}
...
T *uninitialized_copy(const T *first, const T *last, T *result) {
  std::cout << "const\n";
  return yasli_nstd::type_selector<
      yasli_nstd::is_memcopyable<T>::value != 0,
      _impl::uninitialized_memcopier<T>,
      _impl::uninitialized_safe_copier<const T *, T *>>::result::
      execute(first, last, result);
}

template <class T> T *uninitialized_copy(T *first, T *last, T *result) {
  std::cout << "non-const\n";
...
void uninitialized_fill(ForwardIterator first, ForwardIterator last,
  ...
  struct ScopeGuard {
    ...
    ~ScopeGuard() {
      if (pCrt)
        yasli_nstd::destroy_range(first, *pCrt);
    }
  } guard = {first, &first};

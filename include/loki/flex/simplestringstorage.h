...
template <typename E, class A = std::allocator<E>> class SimpleStringStorage {
...
public:
  ...
  SimpleStringStorage(const A &) { pData_ = const_cast<Data *>(&emptyString_); }
  ...
  iterator begin() { return pData_->buffer_; }

  const_iterator begin() const { return pData_->buffer_; }

  iterator end() { return pData_->pEnd_; }

  const_iterator end() const { return pData_->pEnd_; }

  size_type size() const { return pData_->pEnd_ - pData_->buffer_; }

  size_type max_size() const {
    return size_t(-1) / sizeof(E) - sizeof(Data) - 1;
  }

  size_type capacity() const { return pData_->pEndOfMem_ - pData_->buffer_; }
  ...
  template <class InputIterator> void append(InputIterator b, InputIterator e) {
  ...
  void swap(SimpleStringStorage &rhs) { std::swap(pData_, rhs.pData_); }
  ...
  const E *data() const { return pData_->buffer_; }

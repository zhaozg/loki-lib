...
private:
  ...
  Storage &Data() const { return *reinterpret_cast<Storage *>(buf_); }
  ...
  void MakeUnique() const {
    ...
    new (buf_) Storage(*new (temp.buf_) Storage(Data()),
...
public:
  ...
  CowStringOpt(const allocator_type &a) { new (buf_) Storage(1, 1, a); }
  ...
  void swap(CowStringOpt &rhs) { Data().swap(rhs.Data()); }

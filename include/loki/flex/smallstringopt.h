...
// The threshold for SmallStringOpt cannot be greater than 126.  See Loki bug
// #2666059 for more details:
...
#define LOKI_FLEX_STRING_BUFFERSIZECANNOTBEGREATERTHAN126(expression)          \
  {                                                                            \
    SmallStringOptImpl::BufferSizeCannotBeGreaterThan126<((expression) != 0)>  \
        bufferSizeCannotBeGreaterThan126;                                      \
    (void)bufferSizeCannotBeGreaterThan126;                                    \
  }
} // namespace SmallStringOptImpl
...
private:
  enum {
    temp1 = threshold * sizeof(value_type) > sizeof(Storage)
...
private:
  ...
  bool Small() const { return buf_[maxSmallString] != magic; }
...
public:
  ...
  iterator end() {
    if (Small())
      return buf_ + maxSmallString - buf_[maxSmallString];
  ...
  size_type size() const {
    ...
    return Small() ? maxSmallString - buf_[maxSmallString]
                   : GetStorage().size();
  }
  ...
  template <class FwdIterator> void append(FwdIterator b, FwdIterator e) {
    if (!Small()) {
      GetStorage().append(b, e);
    } else {
      // append to a small string
      const size_type sz = std::distance(b, e),
                      neededCapacity =

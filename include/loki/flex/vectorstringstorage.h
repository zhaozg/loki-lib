...
public: // protected:
  ...
  iterator end() { return base::end() - 1; }

  const_iterator end() const { return base::end() - 1; }

  size_type size() const { return base::size() - 1; }

  size_type max_size() const { return base::max_size() - 1; }

  size_type capacity() const { return base::capacity() - 1; }
  ...
  void append(ForwardIterator b, ForwardIterator e) {
    const typename std::iterator_traits<ForwardIterator>::difference_type sz =
        std::distance(b, e);
    assert(sz >= 0);
    if (sz == 0)
      return;
    ...
    struct OnBlockExit {
      ...
      ~OnBlockExit() { that->base::push_back(value_type()); }
    } onBlockExit = {this};
    (void)onBlockExit;
    assert(!base::empty());
  ...
  void swap(VectorStringStorage &rhs) { base::swap(rhs); }

  const E *c_str() const { return &*begin(); }

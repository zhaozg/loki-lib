...
Reallocate(Allocator &alloc, typename Allocator::pointer p,
           typename Allocator::size_type oldObjCount,
           typename Allocator::size_type newObjCount, void *) {
  // @@@ not implemented
}
...
Reallocate(Allocator &alloc, typename Allocator::pointer p,
           typename Allocator::size_type oldObjCount,
           typename Allocator::size_type newObjCount, mallocator<void> *) {
  // @@@ not implemented
}
...
template <typename E, class T = std::char_traits<E>,
...
class flex_string : private Storage {
  ...
  struct Invariant {
#ifndef NDEBUG
    Invariant(const flex_string &s) : s_(s) { assert(s_.Sane()); }
    ~Invariant() { assert(s_.Sane()); }
...
public:
  ...
  flex_string(const flex_string &str, size_type pos, size_type n = npos,
              const A &a = A())
      : Storage(a) {
    assign(str, pos, n);
  }
  ...
  void resize(size_type n, value_type c) { Storage::resize(n, c); }
...
private:
  ...
  //
  // Users can implement this function in their namespace if their storage uses
  // a special iterator type, the function will be found through ADL.
  ...
  // implementations must have side effects. A const version wouldn't make the
  // string unique through this call.
  template <class Iterator>
  ...
  flex_string &InsertImplDiscr(iterator p, size_type n, value_type c,
                               Selector<1>) {
    Invariant checker(*this);
    (void)checker;
    ...
    if (n < originalSize - insertOffset) {
      ...
      // The characters that are pushed back need to be moved because they're
      // aliased. The appended characters will all be overwritten by the move.
      append(n, value_type(0));
      value_type *begin(&*begin());
      flex_string_details::pod_move(begin + insertOffset, begin + originalSize,
                                    begin + insertOffset + n);
      std::fill(begin + insertOffset, begin + insertOffset + n, c);
    } else {
      // The new characters exceed the original string.
      // The characters that are pushed back can simply be copied since they
      // aren't aliased. The appended characters will partly be overwritten by
      // the copy.
      append(n, c);
      value_type *begin(&*begin());
      flex_string_details::pod_copy(begin + insertOffset, begin + originalSize,
                                    begin + insertOffset + n);
  ...
  template <class InputIterator>
  flex_string &InsertImplDiscr(iterator i, InputIterator b, InputIterator e,
                               Selector<0>) {
    InsertImpl(
        i, b, e,
  ...
  void InsertImpl(iterator i, FwdIterator s1, FwdIterator s2,
    ...
    if (IsAliasedRange(s1, s2)) {
      // The source range is contained in the current string, copy it and
      // recurse.
      const flex_string temporary(s1, s2);
      InsertImpl(
          i, temporary.begin(), temporary.end(),
  ...
  template <class InputIterator>
  void InsertImpl(iterator insertPosition, InputIterator inputBegin,
                  InputIterator inputEnd, std::input_iterator_tag) {
...
public:
  ...
  // Replaces at most n1 chars of *this, starting with pos1 with the content of
  // str
  ...
  /*
          ...
          const value_type* s1, const size_type n2)
      {
          ...
          if (delta <= 0)
          {
              ...
              pod_move(&*begin() + pos + n1, &*end(), &*begin() + pos + n1 +
     delta); resize(size() + delta); return *this;
          }
  ...
  */
  ...
  // Replaces at most n1 chars of *this, starting with pos, with n2 occurences
  // of c consolidated with Replaces at most n1 chars of *this, starting with
  // pos, with at most n2 chars of str. str must have at least n2 chars.
  template <class StrOrLength, class NumOrChar>
  flex_string &replace(size_type pos, size_type n1, StrOrLength s_or_n2,
                       NumOrChar n_or_c) {
    Invariant checker(*this);
    (void)checker;
...
private:
  flex_string &ReplaceImplDiscr(iterator i1, iterator i2, const value_type *s,
                                size_type n, Selector<2>) {
    assert(i1 <= i2);
  ...
  flex_string &ReplaceImplDiscr(iterator i1, iterator i2, size_type n2,
                                value_type c, Selector<1>) {
  ...
  template <class InputIterator>
  flex_string &ReplaceImplDiscr(iterator i1, iterator i2, InputIterator b,
                                InputIterator e, Selector<0>) {
    ReplaceImpl(
        i1, i2, b, e,
        typename std::iterator_traits<InputIterator>::iterator_category());
    return *this;
...
public:
  ...
  flex_string &replace(iterator i1, iterator i2, T1 first_or_n_or_s,
                       T2 last_or_c_or_n) {
  ...
  size_type copy(value_type *s, size_type n, size_type pos = 0) const {
    ...
    flex_string_details::pod_copy(data() + pos, data() + pos + n, s);
    return n;
  }
...
std::basic_istream<typename flex_string<E, T, A, S>::value_type,
                   typename flex_string<E, T, A, S>::traits_type> &
getline(std::basic_istream<typename flex_string<E, T, A, S>::value_type,
                           typename flex_string<E, T, A, S>::traits_type> &is,
        flex_string<E, T, A, S> &str,
        typename flex_string<E, T, A, S>::value_type delim) {
  size_t nread = 0;
  typename basic_istream<typename flex_string<E, T, A, S>::value_type,
                         typename flex_string<E, T, A, S>::traits_type>::sentry
      sentry(is, true);
  if (sentry) {
    basic_streambuf<typename flex_string<E, T, A, S>::value_type,
                    typename flex_string<E, T, A, S>::traits_type> *buf =
        is.rdbuf();
    str.clear();

    while (nread < str.max_size()) {
      int c1 = buf->sbumpc();
      if (flex_string<E, T, A, S>::traits_type::eq_int_type(
              c1, flex_string<E, T, A, S>::traits_type::eof())) {
...
const typename flex_string<E1, T, A, S>::size_type
    flex_string<E1, T, A, S>::npos =
        static_cast<typename flex_string<E1, T, A, S>::size_type>(-1);

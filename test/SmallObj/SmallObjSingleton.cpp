...
typedef Loki::SmallValueObject<LOKI_DEFAULT_THREADING_NO_OBJ_LEVEL, 512, 32, 4,
...
public:
  ...
  SmallObjectChild(void) { cout << "SmallObjectChild created" << endl; }
  ~SmallObjectChild(void) { cout << "~SmallObjectChild" << endl; }
  void DoThat(void) { cout << "SmallObjectChild::DoThat" << endl << endl; }
...
public:
  ...
  LongLivedSingleton(void) { cout << "LongLivedSingleton created" << endl; }
  ~LongLivedSingleton(void) { cout << "~LongLivedSingleton" << endl; }
  void DoThat(void) { cout << "LongLivedSingleton::DoThat" << endl << endl; }
...
public:
  typedef Loki::SingletonHolder<
      FollowerSingleton, Loki::CreateUsingNew,
      Loki::FollowIntoDeath::AfterMaster<
          FollowerSingleton::ObjAllocatorSingleton>::IsDestroyed>
      MySmallSingleton;
  ...
  FollowerSingleton(void) { cout << "FollowerSingleton created" << endl; }
  ~FollowerSingleton(void) { cout << "~FollowerSingleton" << endl; }
  void DoThat(void) { cout << "FollowerSingleton::DoThat" << endl << endl; }
...
typedef Loki::SmallValueObject<
    LOKI_DEFAULT_THREADING_NO_OBJ_LEVEL, LOKI_DEFAULT_CHUNK_SIZE,
    LOKI_MAX_SMALL_OBJECT_SIZE, LOKI_DEFAULT_OBJECT_ALIGNMENT, Loki::NoDestroy>
    ImmortalObject;
...
public:
  ...
  ImmortalSingleton(void) { cout << "ImmortalSingleton created" << endl; }
  ~ImmortalSingleton(void) {
    cout << "~ImmortalSingleton destructor should never get called!" << endl;
  }
  void DoThat(void) { cout << "ImmortalSingleton::DoThat" << endl << endl; }
...
public:
  ...
  MortalSingleton(void) { cout << "MortalSingleton created" << endl; }
  ~MortalSingleton(void) { cout << "~MortalSingleton" << endl; }
  void DoThat(void) { cout << "MortalSingleton::DoThat" << endl << endl; }
...
int main() {
  ...
  cout << endl
       ...
       << "Small-Object Allocator and singleton objects that are derived "
       << endl
       ...
       << "at exit time, then all policies work." << endl
       << endl;

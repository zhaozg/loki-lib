...
template <unsigned int N> class ThisIsASmallObject {
  char data[N];
};
...
template <unsigned int N> class BoostPoolNew : public Base<N, void> {
private:
...
#define LOKI_SMALLOBJ_BENCH_ARRAY(FUNC, CODE_DECL, CODE_NEW, CODE_DELETE)      \
  ...
  int FUNC(void **arrv, const int N, int loop, Timer &t, const char *s) {      \
    ...
    if (array_test_nr == LOKI_BOOST_TEST_NR) {                                 \
      cout << "2. boost    :\tboost::object_pool is not tested because it's "  \
              "too slow"                                                       \
           << endl                                                             \
           << endl;                                                            \
      array_test_nr++;                                                         \
...
LOKI_SMALLOBJ_BENCH(delete_new_all, std::allocator<T> st;
                    st.deallocate(st.allocate(1), 1);)

LOKI_SMALLOBJ_BENCH(delete_new_array, delete[] new T[N];)
LOKI_SMALLOBJ_BENCH(delete_new_array_mal,
                    std::free(std::malloc(sizeof(T[TN])));)
LOKI_SMALLOBJ_BENCH(delete_new_array_all, std::allocator<T[TN]> st;
                    st.deallocate(st.allocate(1), 1);)

LOKI_SMALLOBJ_BENCH_ARRAY(new_del_on_arr, , arr[i] = new T;, delete arr[i];)
LOKI_SMALLOBJ_BENCH_ARRAY(new_del_on_arr_mal, ,
                          arr[i] = static_cast<T *>(std::malloc(sizeof(T)));
                          , std::free(arr[i]);)
...
template <unsigned int Size, int loop,
          template <class, class> class ThreadingModel, std::size_t chunkSize,
          std::size_t maxSmallObjectSize, std::size_t objectAlignSize,
          template <class> class LifetimePolicy, class MutexPolicy>
void testSize() {

  typedef Base<Size, void> A;
  typedef Base<Size,
               Loki::SmallObject<ThreadingModel, chunkSize, maxSmallObjectSize,
                                 objectAlignSize, LifetimePolicy, MutexPolicy>>
      B;
  typedef Base<Size, Loki::SmallValueObject<ThreadingModel, chunkSize,
                                            maxSmallObjectSize, objectAlignSize,
                                            LifetimePolicy, MutexPolicy>>
      C;
  ...
  cout << endl << endl;
  cout << "Allocator Benchmark Tests with " << Size << " bytes big objects "
       << endl;
  cout << endl;
  cout << "new      = global new/delete     \tsizeof(A) = " << sizeof(A)
       << endl;
  cout << "SmallObj = Loki::SmallObject     \tsizeof(B) = " << sizeof(B)
       << endl;
  cout << "ValueObj = Loki::SmallValueObject\tsizeof(C) = " << sizeof(C)
       << endl;
#ifdef COMPARE_BOOST_POOL
  cout << "boost    = boost::object_pool    \tsizeof(D) = " << sizeof(D)
       << endl;
#endif
  cout << "allocator= std::allocator        \tsizeof(A) = " << sizeof(A)
       << endl;
  cout << "malloc   = std::malloc/free      \tsizeof(A) = " << sizeof(A)
       << endl;
  cout << endl << endl;
  ...
  cout << "N=" << N << " :  " << loop << " times ";
  LOKI_SMALL_OBJECT_BENCH_ABCD(delete_new_array, N, loop, t,
                               "'delete[] new T[N]'");
  assert((!AllocatorSingleton::IsCorrupted()));

  cout << "i=0..." << Narr << " :  ";
  LOKI_SMALL_OBJECT_BENCH_ABCD(new_del_on_arr, 0, Narr, t,
                               "1. 'arr[i] = new T'   2. 'delete arr[i]'");
  assert((!AllocatorSingleton::IsCorrupted()));

  cout << "i=0..." << Narr << ",  N=" << N << " :  ";
  LOKI_SMALL_OBJECT_BENCH_ABCD(new_del_a_on_a, N, Narr, t,
                               "1. 'arr[i] = new T[N]'   2. 'delete[] arr[i]'");
  assert((!AllocatorSingleton::IsCorrupted()));
...
void DoSingleThreadTest(void) {
  ...
  cout << endl;
  testSize<2, loop, ::Loki::SingleThreaded, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<3, loop, ::Loki::SingleThreaded, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<8, loop, ::Loki::SingleThreaded, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<9, loop, ::Loki::SingleThreaded, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<16, loop, ::Loki::SingleThreaded, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<17, loop, ::Loki::SingleThreaded, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
}
...
void DoClassLockTest(void) {
  const int loop = 1000 * 1000;
  cout << endl;
  testSize<2, loop, ::Loki::ClassLevelLockable, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<3, loop, ::Loki::ClassLevelLockable, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<8, loop, ::Loki::ClassLevelLockable, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<9, loop, ::Loki::ClassLevelLockable, 4096, 128, 4, ::Loki::NoDestroy,
           ::Loki::Mutex<std::mutex>>();
  testSize<16, loop, ::Loki::ClassLevelLockable, 4096, 128, 4,
           ::Loki::NoDestroy, ::Loki::Mutex<std::mutex>>();
  testSize<17, loop, ::Loki::ClassLevelLockable, 4096, 128, 4,
           ::Loki::NoDestroy, ::Loki::Mutex<std::mutex>>();
}

...
public:
...
#endif
  {
    ...
    if (s_verbose)
      cout << "static void * operator new ( std::size_t size ) throw ( "
              "std::bad_alloc )"
           << endl
           << "\tsize = " << size << endl;
    cout << endl;
  ...
  static void *operator new(std::size_t size, void *place) {
    ...
    if (s_verbose)
      cout << "static void * operator new ( std::size_t size, void * place )"
           << endl
           << "\tsize = " << size << endl
           << "\tplace = " << place << endl
           << "\treturn = " << place2 << endl;
  ...
  //        if ( s_verbose )
  //            cout << "static void operator delete ( void * place ) throw ()"
  //            << endl
  //                 << "\tplace = " << place << endl;
  //        cout << endl;
  ...
  static void operator delete(void *place, std::size_t size) throw() {
    ...
    if (s_verbose)
      cout << "static void operator delete ( void * place, std::size_t size ) "
              "throw ()"
           << endl
           << "\tplace = " << place << endl
           << "\tsize = " << size << endl;
  ...
  static void operator delete(void *place1, void *place2) {
    ...
    if (s_verbose)
      cout << "static void operator delete ( void * place1, void * place2 )"
           << endl
           << "\tplace1 = " << place1 << endl
           << "\tplace2 = " << place2 << endl;
    cout << endl;
  ...
  static void operator delete[](void *place, std::size_t size) throw() {
    ...
    if (s_newArraySize != size) {
      cout << "\tERROR: The size parameter should be: " << s_newArraySize << '!'
           << endl;
      s_errorCount++;
    }
    cout << endl;
...
void Bee::OutputArraySizeInfo(void) {
  ...
  if ((0 == s_newArraySize) || (0 == s_deleteArraySize))
    cout << "I do not have sufficient information to know if your compiler "
            "complies"
         << endl
         << "with Section 12.5/5 of the C++ Standard.  You should not use Loki "
            "to"
         << endl
         << "allocate arrays.  Please do not #define "
            "LOKI_SMALL_OBJECT_USE_NEW_ARRAY."
         << endl;
  else if (s_newArraySize == s_deleteArraySize)
    cout << "Your compiler provides correct value for delete [] operator.  You "
            "may use"
         << endl
         << "Loki to allocate arrays via #define "
            "LOKI_SMALL_OBJECT_USE_NEW_ARRAY.  If"
         << endl
         << "you do, please run the SmallObjBench tests to see if Loki is "
            "faster or"
         << endl
         << "slower at allocating arrays than the default allocator." << endl
         << endl;
  else
    cout << "Your compiler does NOT provide correct size value for delete [] "
            "operator."
         << endl
         << "It should provide the same size value to delete [] as it did to "
            "new []."
         << endl
         << "Please ask your compiler vendor to comply with Section 12.5/5 of "
            "the C++"
         << endl
         << "Standard.  You should NOT use Loki to allocate arrays.  Please do "
            "*not*"
         << endl
...
void OutputCompilerType(void) {
  ...
  if (zeroSize) {
    cout << "Your compiler allows datatypes to be zero bytes, which is "
            "contrary to"
         << endl;
  ...
  cout << "( sizeof(EmptyThing) == " << sizeof(EmptyThing) << " )" << endl
       << endl;
}
...
int main(int argc, const char *const argv[]) {
  ...
  if (caught) {
    ...
    cout << "Tests of default new and delete operators aborted because of "
            "exception!"
         << endl;
  }

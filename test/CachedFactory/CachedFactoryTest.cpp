...
#include <windows.h>
milliSec getmilliSeconds() { return clock() * 1000 / CLOCKS_PER_SEC; }
#else
#include <unistd.h>
...
public:
  DebugProduct() { cout << "Product Ctor()" << endl; }
  DebugProduct(int, int) { cout << "Product Ctor(int, int)" << endl; }
  ~DebugProduct() { cout << "Product Dtor()" << endl; }
};

CostlyProduct *createCostlyProductNull() { return new CostlyProduct; }

DebugProduct *createDebugProductNull() { return new DebugProduct; }

Product *createProductNull() { return new Product; }

Product *createProductInt(int a, int b) { return new Product(a, b); }
...
template <class CCache> bool unitTestCacheOverhead(int loop) {
  ...
  cout << " | average overhead per fetch : "
       << ((static_cast<double>(elapsedCache - elapsedNoCache)) /
           CLOCKS_PER_SEC * 1000 / loop)
       << " ms" << endl;
  return true;
}
...
void testCacheOverhead() {
  ...
  typedef CachedFactory<AbstractProduct, int, NullType, SimplePointer,
      ...
      CAgingEvict;
  bool test1 =
      dispResult("Random policy", unitTestCacheOverhead<CRandomEvict>(loop));
  (void)test1;
  bool test2 = dispResult("LRU policy", unitTestCacheOverhead<CLRUEvict>(loop));
  (void)test2;
  bool test3 =
      dispResult("Aging policy", unitTestCacheOverhead<CAgingEvict>(loop));
  (void)test3;
}
void unitTestCachePerformance(int loop) {
  ...
  elapsedCache = getmilliSeconds() - start;
  cout << "No cache " << elapsedNoCache << " ms | Cache " << elapsedCache
       << " ms | Efficiency " << (double(elapsedNoCache) / elapsedCache) - 1
       << endl;
}
...
milliSec typicalUse(Cache &CC, unsigned objectKind, unsigned maxObjectCount,
  ...
  try {
    ...
    for (unsigned i = 0; i < maxIteration; i++) {
      ...
      if (size == maxObjectCount) {
        ...
        fetched.push_back(
            CC.CreateObject(int(objectKind * rand() / (RAND_MAX + 1.0))));
  ...
  for (std::vector<AbstractProduct *>::iterator itr = fetched.begin();
       itr != fetched.end(); itr++)
...
template <class Cache>
void displayTypicalUse(Cache &CC, unsigned objectKind, unsigned maxObjectCount,
                       unsigned maxIteration) {
  CC.displayCacheType();
  cout << "====> "
       << typicalUse<Cache>(CC, objectKind, maxObjectCount, maxIteration)
       << " ms" << endl;
}
...
void testTypicalUse() {
  ...
  cout << "# Test performs " << maxIteration << " iterations" << endl;
  {
    typedef CachedFactory<AbstractProduct, int, NullType, SimplePointer,
        ...
        CRandomEvict;
    CRandomEvict cache;
    displayTypicalUse<CRandomEvict>(cache, objectKind, maxObjectCount,
                                    maxIteration);
  }
  {
    typedef CachedFactory<AbstractProduct, int, NullType, SimplePointer,
...
template <class Cache> bool testEvictionError() {
...
bool fullTestRateLimitedFetchPolicy() {
  ...
  bool test1 =
      dispResult("Fast creation", testRateLimitedFetchPolicy(false) == true);
  cout << " || => generating 5 objects with 70ms between each Fetch" << endl;
  bool test2 =
      dispResult("Slow creation", testRateLimitedFetchPolicy(true) == false);
  return test1 && test2;
}
...
void separator() { cout << endl << endl; }

void performanceTests() {
  dispText(" ==> Performance tests <==");
  separator();
  dispText("Test typical use",
           "tries different Cache strategies in a typical use configuration");
  testTypicalUse();
  separator();
  dispText(
      "Test efficiency",
      "Comparison between generating costly objects (100ms) and using Cache");
  testCachePerformance();
  separator();
  dispText("Test overhead",
           "Intensive use of Cache to determine the cache time overhead");
  testCacheOverhead();
  separator();
}
...
void reliabilityTests() {
  ...
  separator();
  dispText("Test caching", "Trying to Create, Release, Create and see if Cache "
                           "gives the same object");
  bool cacheResult = dispResult("caching result", testCache());
  separator();
  dispText("Test RateLimitedFetch policy",
           "Trying to quickly create objects, then same scenario with pause in "
           "between");
  bool rateLimitedResult = dispResult("RateLimitedFetch policy result",
                                      fullTestRateLimitedFetchPolicy());
  separator();
  dispText("Test AmountLimitedCreation policy",
           "Trying to create 2 objects with a limit of 1 creation max, you "
           "should see a destroyed object (eviction)");
  bool amountLimitedResult = dispResult("AmountLimitedCreation policy result",
                                        testAmountLimitedCreation());
  separator();
  dispText(
      "Test eviction error",
      "An eviction should occur (Creation Policy), but all object are in use");
  bool evictionTest =

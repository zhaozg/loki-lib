...
typedef Loki::SmartPtr<Thingy, RefCounted, DisallowConversion, NoCheck,
                       DefaultSPStorage, PropagateConst>
...
typedef Loki::SmartPtr<Thingy, RefCounted, DisallowConversion, AssertCheck,
                       HeapStorage, PropagateConst>
    Thingy_HeapStorage_ptr;

// ----------------------------------------------------------------------------

/// @note Used for testing most policies.
typedef Loki::SmartPtr<BaseClass, RefCounted, DisallowConversion, AssertCheck,
                       DefaultSPStorage, DontPropagateConst>
    NonConstBase_RefCount_NoConvert_Assert_DontPropagate_ptr;

/// @note These 4 are used for testing const policies.

typedef Loki::SmartPtr<BaseClass, RefCounted, DisallowConversion, NoCheck,
                       DefaultSPStorage, DontPropagateConst>
    NonConstBase_RefCount_NoConvert_NoCheck_DontPropagate_ptr;

typedef Loki::SmartPtr<const BaseClass, RefCounted, DisallowConversion, NoCheck,
                       DefaultSPStorage, DontPropagateConst>
...
typedef Loki::SmartPtr<BaseClass, RefLinked, DisallowConversion, AssertCheck,
                       DefaultSPStorage, DontPropagateConst>
    NonConstBase_RefLink_NoConvert_Assert_DontPropagate_ptr;

typedef Loki::SmartPtr<BaseClass, DeepCopy, DisallowConversion, AssertCheck,
                       DefaultSPStorage, DontPropagateConst>
...
typedef Loki::SmartPtr<BaseClass, NoCopy, DisallowConversion, AssertCheck,
                       DefaultSPStorage, DontPropagateConst>
...
typedef Loki::SmartPtr<MimicCOM, COMRefCounted, DisallowConversion, AssertCheck,
                       DefaultSPStorage, DontPropagateConst>
    MimicCOM_ptr;
...
void DoOwnershipConversionTests(void) {
...

  {
    ...
    // illegal constructions!  Can't convert from one ownership policy to
    // another. NonConstBase_RefCount_NoConvert_Assert_DontPropagate_ptr p13( p2
    ...
    // illegal constructions!  Can't convert from one ownership policy to
    // another. NonConstBase_ComRef_NoConvert_Assert_DontPropagate_ptr p18( p1
    ...
    // illegal constructions!  Can't convert from one ownership policy to
    // another. NonConstBase_RefLink_NoConvert_Assert_DontPropagate_ptr p23( p1
    ...
    // illegal constructions!  Can't convert from one ownership policy to
    // another. NonConstBase_DeepCopy_NoConvert_Assert_DontPropagate_ptr p28( p1
    ...
    // illegal constructions!  Can't convert from one ownership policy to
    // another. NonConstBase_KillCopy_NoConvert_Assert_DontPropagate_ptr p33( p1
    ...
    // illegal constructions!  Can't convert from one ownership policy to
    // another. NonConstBase_NoCopy_NoConvert_Assert_DontPropagate_ptr p38( p1
...
void DoInheritanceConversionTests(void) {
...

  {
    ...
    assert(p1 == p2);
    //      p1 = p3;  // illegal!  Can't assign pointer since base class is
    //      private. p2 = p1;  // illegal!  Can't do cast to derived class in
    //      pointer assignment. p2 = p3;  // illegal!  Can't assign when types
    //      are unrelated. p3 = p1;  // illegal!  Can't do cast to derived class
    //      in pointer assignment. p3 = p2;  // illegal!  Can't assign when
    //      types are unrelated.
...
void DoSmartPtrDynamicCastTests(void) {
  ...
  typedef ::Loki::SmartPtr<Feline, RefCounted, DisallowConversion, NoCheck>
      FelinePtr;
  typedef ::Loki::SmartPtr<Tiger, RefCounted, DisallowConversion, NoCheck>
      TigerPtr;
  typedef ::Loki::SmartPtr<Lion, RefCounted, DisallowConversion, NoCheck>
      LionPtr;
...

  {
    ...
    // This is legal because C++ allows an automatic down-cast to public base
    // class.
    pFeline = pLion;

#ifdef CHECK_TYPE_CAST
    pLion =
        pFeline; // Fails as the compiler cannot convert pointers in SmartPtr
    ...
    // Can't cast from lion to tiger since although these are both types of
    // felines, they are not related to one another.
...
/// @note Used for testing exception-throwing policy, and for checking for
/// null-pointer dereference.
typedef ::Loki::SmartPtr<Feline, DeepCopy, DisallowConversion, RejectNullStatic,
                         DefaultSPStorage, DontPropagateConst>
    Feline_DeepCopy_Ptr;
typedef ::Loki::SmartPtr<Tiger, DeepCopy, DisallowConversion, RejectNullStatic,
                         DefaultSPStorage, DontPropagateConst>
    Tiger_DeepCopy_Ptr;
typedef ::Loki::SmartPtr<Lion, DeepCopy, DisallowConversion, RejectNullStatic,
                         DefaultSPStorage, DontPropagateConst>
    Lion_DeepCopy_Ptr;
typedef ::Loki::SmartPtr<Dog, DeepCopy, DisallowConversion, RejectNullStatic,
                         DefaultSPStorage, DontPropagateConst>
    Dog_DeepCopy_Ptr;
...
void DoDestructiveCopyTest(void) {
...

  {
    ...
    assert(p1);
    /** @note The following code won't compile because p1 is declared const. You
     can test if Loki's SmartPtr DestructiveCopy policy was designed correctly
     by uncommenting these lines and seeing if any errors occur when compiling.
     If you see errors about converting from const to non-const, or about
     assigning a read-only reference, then DestructiveCopy was designed
     correctly.
     */
    // DestructiveCopyPtr p2;
    // assert( !p2 );
  ...
  {
    /// @todo The following lines need to be uncommented when bug 3224572 gets
    /// fixed.
...
typedef Loki::SmartPtr<Tiger, RefCounted, DisallowConversion, AssertCheck,
                       ArrayStorage, DontPropagateConst>
    TigerArray_RefCounted_ptr;

typedef Loki::SmartPtr<Tiger, RefLinked, DisallowConversion, AssertCheck,
                       ArrayStorage, DontPropagateConst>
...
bool Compare(const Ptr &, const Ptr &) { return true; }
...
void friend_handling() {
  ...
  std::search(vec.begin(), vec.end(), vec.begin(), vec.end(), Compare);
  Ptr a, b;
  Compare(a, b);
...
// by default Loki is compiled without thread support, so we must disable the
// dependency on thread classes (StrongPtr) to avaoid linker errors when
// compiling with the default build process. Should  we change the default
// threading of Loki?
//

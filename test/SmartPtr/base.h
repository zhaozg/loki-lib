...
class BaseClass {
public:
  BaseClass(void) : m_refCount(1) { s_constructions++; }

  virtual ~BaseClass(void) { s_destructions++; }
  ...
  void Release(void) {
    ...
    --m_refCount;
    if (0 == m_refCount)
      delete this; // Don't even ask me how much I dislike seeing "delete
                   // this;"!
  }

  // This function is used only for the DeepCopy policy.
  virtual BaseClass *Clone(void) const { return new BaseClass(); }
...
public:
  // This function is used only for the DeepCopy policy.
  virtual BaseClass *Clone(void) const { return new PublicSubClass; }
};
...
public:
  // This function is used only for the DeepCopy policy.
  virtual BaseClass *Clone(void) const { return new PrivateSubClass; }
};

// ----------------------------------------------------------------------------

/** @class Feline - The feline family of classes are to test dynamic_cast. Also
 used to test pointers to arrays of objects.
 */
...
public:
  ...
  virtual ~MimicCOM(void) { s_destructions++; }
...
private:
  ...
  MimicCOM(void) : m_count(0), m_AddRefCount(0), m_ReleaseCount(0) {
    s_constructions++;
  }

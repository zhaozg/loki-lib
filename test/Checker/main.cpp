...
private:
  ...
  // This shows how to declare checkers for non-static functions in a host
  // class.
...
Thingy::Thingy(unsigned int value) : m_value(value), m_counts() {
...
Thingy::Thingy(const Thingy &that)
    : m_value(that.m_value), m_counts(that.m_counts) {
  CheckFor::Invariants checker(this, &Thingy::IsValid);
  (void)checker;
}
...
Thingy::~Thingy(void) { assert(IsValid()); }
...
void Thingy::DoSomethingEvil(void) { m_value = 0; }
...
void Thingy::AddCount(unsigned int count) {
  ...
  CheckFor::Invariants checker(this, &Thingy::IsValid, nullptr,
                               &Thingy::IsValidFull);
...
unsigned int Thingy::GetCount(unsigned int index) const {
  ...
  CheckMementoFor::NoThrowOrChange checker(
      this, &Thingy::IsValid, &Thingy::IsValidFull, &Thingy::IsValidFull);
...
void Thingy::ClearCounts(void) {
  ...
  CheckFor::NoThrow checker(this, &Thingy::IsValid, nullptr,
                            &Thingy::IsValidEmpty);
  m_counts.clear();
}
...
int main(int argc, const char *const argv[]) {
  ...
  (void)argc;
  (void)argv;
  cout << "These tests confirm the Checker exception policies work properly."
       << endl;
  cout << "The tests will throw exceptions. If you see any assertions, the "
          "test failed."
       << endl;
  ...
  try {
    cout << "Starting tests of CheckFor::Invariants and "
            "CheckFor::NoThrowOrChange policy."
         << endl;
    Thingy t1(1);
  ...
  cout << "Finished tests of CheckFor::Invariants and "
          "CheckFor::NoThrowOrChange policy."
       << endl;
  ...
  cout << "All done!  If you see this line, and no assertions failed, then the "
          "test passed!"
       << endl;
  return 0;
}

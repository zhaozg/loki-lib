...
Integral2 RandomInt(Integral1 low, Integral2 up) {
  ...
  const Integral2 low2 = low, n = up - low;
...
template <class T> void TestCase(const string &fmt, T value) {
  char buf[4096];
  std::string s;
  ...
  if (i1 != i2 || s != buf) {
    cout << endl
         << "Reference: " << i2 << "; Actual: " << i1
         << ",  Difference = " << i2 - i1 << endl
         << "V: [" << value << "]" << endl
         << "F: [" << fmt << "]" << endl
         << "R: [" << buf << "]" << endl
         << "A: [" << s.c_str() << "]" << endl;
...
void AtomicTest(void) {
  ...
  cout << "If the output lines up in neat columns, your compiler implements "
          "printf correctly."
       << endl;
  cout << "If the output is not in columns, then your compiler implements "
          "printf incorrectly."
       << endl;
  ...
  cout << "If the output lines up in neat columns, your compiler implements "
          "cout correctly."
       << endl;
  cout << "If the output is not in columns, then your compiler implements cout "
          "incorrectly."
       << endl;
}
...
public:
  ...
  inline unsigned int GetRandomLoopCount(void) const {
    return m_randomLoopCount;
  }
...
CommandLineArgs::CommandLineArgs(unsigned int argc, const char *argv[])
    : m_speedLoopCount(0), m_randomLoopCount(0), m_doFormatTest(false),
      m_doAtomicTest(false), m_showHelp(false), m_exeName(argv[0]) {
...
void CommandLineArgs::ShowHelp(void) const {
  ...
  cout << " -h    Show this help info and exit. Overrides all other options."
       << endl;
  cout << " -f    Run formatting tests." << endl;
  cout << " -a    Run atomic tests." << endl;
  cout << " -r:#  Run random tests for # of loops. # is a positive decimal "
          "value greater than 100."
       << endl;
  cout << " -s:#  Run speed tests for # of loops. # is a positive decimal "
          "value greater than 100."
       << endl;
}

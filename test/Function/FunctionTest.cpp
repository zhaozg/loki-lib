...
#define BOOST_CHECK(exp)                                                       \
  ((exp) ? static_cast<void>(0) : report_error(#exp, __FILE__, __LINE__))
...
static void test_static_function() { static_func(); }
...
static void test_call_obj(function<int(int, int)> f) { assert(!f.empty()); }

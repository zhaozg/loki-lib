...
void Decrement(unsigned int &x) { --x; }

struct UserDatabase {
  void AddFriend(const std::string &, const std::string &) { throw 55; }
};
...
void User::HasOne(int p1) const { (void)p1; }
...
std::string User::GetName() const { return "A name"; }

size_t User::countFriends() const { return friends_.size(); }
...
Junk::Junk(void) { assert(NULL != this); }
...
Junk *Attic::RemoveUselessJunk(void) {
  ...
  ScopeGuard testGuard =
      MakeObjGuard(*this, &Attic::CheckInvariants, __FUNCTION__, __LINE__);
  ...
  for (JunkPileIter it(m_storage.begin()); it != m_storage.end(); ++it) {
    Junk *junk = *it;
    if (junk->IsUgly())
      return junk;
    if (junk->IsUseful())
